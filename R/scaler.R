...
#'
#' x_valid
ation <- data.frame(colors = c('Blue', 'Red', 'Green'), counts = c(29, 65, 13), usage = c(5, 27, 10))
#'
#' x_test <- data.frame(colors = c('Blue', 'Red', 'Green'), counts = c(20, 35, 18), usage = c(9, 6, 0))
#'
#' colnames <- c('counts', 'usage')
#'
#' X_train = scaler(x_train, x_valid
, x_test, colnames)$X_train
#' X_test = scaler(x_train, x_valid
ation, x_test, colnames)$X_test
#' x_valid
ation = scaler(x_train, x_valid
ation, x_test, colnames)$x_valid
ation


scaler <- function(X_train, x_valid
ation, X_test, colnames) {
  library(testthat)
  ...
  if (length(X_train) == 0 | length(X_test) == 0 | length(x_valid
  ation) == 0 | length(colnames) == 0) {
  ...
  x_valid
  ation_scaled <- data.frame(x_valid
  ation)
  x_valid
  ation_scaled[colnames] <- scale(x_valid
  ation[colnames])
  ...
  scaled_data <- list(X_train_scaled, x_valid
  ation_scaled, X_test_scaled)
  names(scaled_data) <- c('X_train', 'x_valid
  ation', 'X_test')
